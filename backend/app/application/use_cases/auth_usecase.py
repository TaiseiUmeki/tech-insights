...
from app.domain.exceptions.business_exceptions import AuthenticationError
...
class AuthUsecase:
    ...
    def login(self, input_dto: LoginInputDTO) -> LoginOutputDTO:
        ...
        if input_dto.login_id == 'admin' and input_dto.password == 'pass':
            ...
            raise AuthenticationError('ログインIDまたはパスワードが正しくありません')
        ...
        #     raise AuthenticationError("ログインIDもしくはパスワードが違います。")
        #
    ...
    def get_auth_status(self, user_id: int) -> StatusOutputDTO:
        """認証状態を取得"""
        return StatusOutputDTO.from_user_id(user_id)
