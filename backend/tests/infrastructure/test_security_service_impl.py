...
class TestSecurityServiceImpl:
    ...
    def test_hash_password_different_results(self, security_service):
        """同じパスワードでも毎回異なるハッシュが生成される"""
        hash1 = security_service.hash_password('same_password')
        hash2 = security_service.hash_password('same_password')
        assert hash1 != hash2
