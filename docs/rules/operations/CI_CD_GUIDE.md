...
└── showcase-sync-check.yml     # UIショーケース同期チェック
```
...
- run: pytest tests/application -v
- run: pytest tests/infrastructure tests/presentation -v
...
### Showcase Sync Check (`showcase-sync-check.yml`)

**トリガー:** `frontend/src/shared/ui/**`, `frontend/src/widgets/**` の変更時（PR）

| ジョブ | 内容 |
|--------|------|
| check | 新規UIコンポーネントがショーケース（/dev/ui）に登録されているか検証 |

```yaml
# 実行例
- run: bash frontend/scripts/check-showcase-sync.sh
```
