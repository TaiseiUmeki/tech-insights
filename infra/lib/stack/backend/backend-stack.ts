...
export class BackendStack extends cdk.Stack {
  ...
  public readonly frontendEcrRepository?: ecr.IRepository;
  ...
  constructor(
    ...
    const frontendEcrConstruct =
      config.frontend.type === 'ecs'
        ? new EcrConstruct(this, 'FrontendEcr', {
            repositoryName: `${config.envName}-cdk-template-frontend`,
            removalPolicy: config.removalPolicy,
            maxImageCount: config.envName === 'prod' ? 20 : 10,
            imageTagMutability: ecr.TagMutability.MUTABLE,
          })
        : undefined;
    this.frontendEcrRepository = frontendEcrConstruct?.repository;
    ...
    const apiResource = new ApiResource(this, 'ApiResource', {
      ...
      frontendEcs:
        config.frontend.type === 'ecs' && frontendEcrConstruct
          ? {
              ecrRepository: frontendEcrConstruct.repository,
              serviceName: `${config.envName}-cdk-template-frontend`,
              cpu: config.ecs.frontend.cpu,
              memoryLimitMiB: config.ecs.frontend.memory,
              desiredCount: config.ecs.frontend.desiredCount,
              containerPort: config.frontend.ecsContainerPort ?? 80,
              pathPatterns: config.frontend.ecsBackendPathPatterns ?? [
                '/api/*',
                '/health',
              ],
            }
    ...
    this.ecsFrontendService = apiResource.ecsFrontendService;
    ...
    if (this.frontendEcrRepository) {
      new cdk.CfnOutput(this, 'FrontendEcrRepositoryUri', {
        value: this.frontendEcrRepository.repositoryUri,
        description: 'Frontend ECR Repository URI',
        exportName: `${config.envName}-FrontendEcrRepositoryUri`,
      });
      new cdk.CfnOutput(this, 'FrontendEcrRepositoryArn', {
        value: this.frontendEcrRepository.repositoryArn,
        description: 'Frontend ECR Repository ARN',
        exportName: `${config.envName}-FrontendEcrRepositoryArn`,
      });
    }
  }
}
