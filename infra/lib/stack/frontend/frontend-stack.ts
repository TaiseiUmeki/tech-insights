...
export interface FrontendStackProps extends cdk.StackProps {
  ...
  webAclId?: string;
  /**
   * ECS フロント使用時: BackendStack の ALB DNS（同一 URL で UI と API をパス分離）
   */
 ...
 * 1. Amplify（デフォルト）
 * 2. S3 + CloudFront
 * 3. ECS（BackendStack 側で Fargate + 同一 ALB。本スタックは URL 出力のみ）
 *
...
export class FrontendStack extends cdk.Stack {
  ...
  constructor(
    ...
    if (config.frontend.type === 'ecs') {
      const albDns = props?.backendAlbDnsName;
      if (!albDns) {
        throw new Error(
          'frontend.type is ecs: pass backendAlbDnsName (BackendStack ALB DNS) in FrontendStack props'
        );
      }
      this.frontendUrl = `http://${albDns}`;

      new cdk.CfnOutput(this, 'FrontendUrl', {
        value: this.frontendUrl,
        description:
          'Frontend URL (same ALB as backend; default path → frontend ECS, API paths → backend ECS)',
        exportName: `${config.envName}-FrontendUrl`,
      });
    } else if (config.frontend.type === 'amplify') {
