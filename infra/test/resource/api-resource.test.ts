...
describe('ApiResource', () => {
  describe('With Lambda Configuration', () => {
    ...
    describe('Lambda Function', () => {
      it('should create a Lambda function', () => {
        // logRetention設定によりCDKがカスタムリソース用Lambdaを追加作成するため2つ
  ...
  describe('With Frontend ECS (dual service + path routing)', () => {
    it('should create two ECS services and two target groups', () => {
      const app = new cdk.App();
      const stack = new cdk.Stack(app, 'TestStack', {
        env: { account: '123456789012', region: 'ap-northeast-1' },
      });
      const vpc = new ec2.Vpc(stack, 'TestVpc', { maxAzs: 2 });
      const feRepo = new ecr.Repository(stack, 'FeRepo', {
        repositoryName: 'test-frontend-ecr',
      });

      const api = new ApiResource(stack, 'TestApi', {
        lambdaFunctionName: 'test-lambda',
        apiGatewayName: 'test-api',
        ecsClusterName: 'test-cluster',
        ecsServiceName: 'test-service',
        ...
        vpc,
        frontendEcs: {
          ecrRepository: feRepo,
          serviceName: 'test-frontend',
          cpu: 256,
          memoryLimitMiB: 512,
          desiredCount: 1,
          containerPort: 80,
          pathPatterns: ['/api/*', '/health'],
        },
      });
      ...
      expect(api.ecsFrontendService).toBeDefined();
      expect(api.frontendEcsSecurityGroup).toBeDefined();
    });
  });
});
