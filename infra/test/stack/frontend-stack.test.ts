...
describe('FrontendStack', () => {
  ...
  describe('ECS mode (URL output only)', () => {
    it('should not create Amplify/S3/CloudFront and export ALB-based URL', () => {
      const app = new cdk.App();
      const ecsFrontendConfig = {
        ...testConfig,
        frontend: {
          ...testConfig.frontend,
          type: 'ecs' as const,
      ...
      const stack = new FrontendStack(
        app,
        'TestFrontendStackEcs',
        ecsFrontendConfig,
        {
          backendAlbDnsName: 'test-alb.ap-northeast-1.elb.amazonaws.com',
          env: {
            account: '123456789012',
            region: 'ap-northeast-1',
          },
      ...
      expect(stack.frontendUrl).toBe(
        'http://test-alb.ap-northeast-1.elb.amazonaws.com'
      );
      const outputs = template.findOutputs('*');
      expect(outputs.FrontendUrl?.Value).toContain('http://');
    });
  });
});
